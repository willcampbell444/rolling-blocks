...
Player::Player(Shaders* shader, int w, int l) {
    ...
    _cameraPos = glm::vec3((-(_floorWidth/2.0f)+_x+((_height/2.0f)*_direction.x)-(_direction.x/2.0f) + 0.5f), (-(_direction.y/2.0f)+0.5f+((_height/2.0f)*_direction.y)), (-(_floorLength/2.0f)+_z+((_height/2.0f)*_direction.z)-(_direction.z/2.0f)+0.5f));
}
...
void Player::move(int x, int z) {
    if (!_isTransition) {
        ...
        _frame = 0;
        _oldCameraPos = glm::vec3((-(_floorWidth/2.0f)+_x+((_height/2.0f)*_direction.x)-(_direction.x/2.0f) + 0.5f), (-(_direction.y/2.0f)+0.5f+((_height/2.0f)*_direction.y)), (-(_floorLength/2.0f)+_z+((_height/2.0f)*_direction.z)-(_direction.z/2.0f)+0.5f));
        ...
        if (_direction.y) {
            ...
            _rotationAxisPosition = glm::vec3(std::abs(x)*(_floorWidth/2.0f - _x - (x/2.0f + 0.5f)), 0, std::abs(z)*(_floorLength/2.0f - _z - (z/2.0f + 0.5f)));
            // std::cout << "noso = " << _z << ", " << _floorWidth/2.0f << std::endl;
            // std::cout << "direction = " << _rotationAxisPosition.x << ", " << _rotationAxisPosition.y << ", " << _rotationAxisPosition.z << std::endl;
        } else {
            if ((_direction.x == x && x) || (_direction.z == z && z)) {
                ...
                _angleSign = std::abs(z)*2 - 1;
                _rotationAxisPosition = glm::vec3(std::abs(x)*(_floorWidth/2.0f - (_x+(x*(_height-1))) - (x/2.0f + 0.5f)), 0, std::abs(z)*(_floorLength/2.0f - (_z+(z*(_height-1))) - (z/2.0f + 0.5f)));
        ...
        _newCameraPos = glm::vec3((-(_floorWidth/2.0f)+_nextX+((_height/2.0f)*_nextDirection.x)-(_nextDirection.x/2.0f) + 0.5f), (-(_nextDirection.y/2.0f) + 0.5f+((_height/2.0f)*_nextDirection.y)), -(_floorLength/2.0f)+_nextZ+((_height/2.0f)*_nextDirection.z)-(_nextDirection.z/2.0f)+0.5f);
    }
}
...
void Player::draw(glm::mat4 viewProjectionMatrix) {
    ...
    for (int i = 0; i < _height; i++) {
        ...
        * glm::translate(glm::mat4(1.0f), glm::vec3((-(_floorWidth/2.0f)+_x+(i*_direction.x)+0.5f)*(1.0/0.9), (0.5f+(i*_direction.y))*(1.0/0.9), (-(_floorLength/2.0f)+_z+(i*_direction.z)+0.5f)*(1.0/0.9)));
        glUniformMatrix4fv(_shader->getUniformLocation("transformMatrix"), 1, GL_FALSE, glm::value_ptr(_transformMatrix));
        glBindVertexArray(_vertexArrayObject);
